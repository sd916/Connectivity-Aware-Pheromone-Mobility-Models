...
def main():
    ...
    beta_type = float(sys.argv[5]) # tuning parameter; set beta to [0.5, 2.0, 3.0, 4.0] for CAP; (0.5 - low connectivity, high coverage), (4.0 - high connectivity, low coverage)
    MOBILILTY_MODEL_TYPE = 'CAP' #sys.argv[6]
...
# for loop every episode
    ...
    for ith_episode in range(num_episodes):
        ...
        set_random_seed(random_seed); print("Seed:", random_seed ) #SHREY*: Added np.random.seed(random_seed)
        ...
        swarm = ph.uav_swarm(env, evaporation_rate = evp, diffusion_rate = diff, use_connect=True, # initaialize simulation environment
                        nAgents=nAgents, map_size=map_size, collision_avoidance=True, collision_buffer=float(ca),
        ...
        save_NodeTrajectories(TRAJECTORY_FOLDER, ith_episode, nAgents, res.UAV_positions, res.UAV_nextwaypoints, simTime-position_log_interval, simTime ) #SHREY*: saving node trajectories for 200s
...
def save_NodeTrajectories(file_path, runNo, nAgents, log_positions, log_nextwaypoints, log_startTime, log_endTime):
    ...
    for node in range(nAgents):
        ...
        np.savetxt(file_path + f"{runNo}CAP_Node{node}.txt", (NODE_LOCATIONS_WAYPOINTS[log_startTime:log_endTime]) / 1e3, delimiter=",", fmt="%.5f")
