...
class UAVStats(object):
    ...
    #----------------------------------------------------------------------------------
        ...
        self.UAV_nextwaypoints = np.zeros((sim_time+1, nAgents+nBaseStations, 2))  
...
class uav_swarm(object):
    ...
    def __init__(self, env, **kwargs):
        ...
        self.evaporation_rate  = kwargs.get('evaporation_rate',    .006)
        self.diffusion_rate    = kwargs.get('diffusion_rate',      .006)
        ...
        self.nBaseStations= kwargs.get('nBaseStations', 1)  # BS-CAP we need atleast 1 BS
        ...
        self.MOBILILTY_MODEL_TYPE = kwargs.get('MOBILILTY_MODEL_TYPE', 'BSCAP')
    ...
    def sim_start_3d_simpy(self, sim_time, drawUAVs=True, drawMap=False, stats=None, plotInterval=10):
        ...
        #12                        self.node_pheromone_Repel[pher_cell+(uavID,)] = 1;   #node_pheromone_Repel(pher_x,pher_y,uavID) + 10 ; %shrey: %NEW increament pheromone by 1
            ...
            #log UAVPositions
            self.stats.UAV_positions[t,:,:] = self.Arobot[0:2, :].T  #SD* Update node locations every time step 
            self.stats.UAV_nextwaypoints[t,:,:] = self.Acontroller[11:13, :].T   #SD* nxtwaypoint = (controller[11], controller[12]); Update node next waypoint locations
    ...
    def plot_UAV_figures(self, t, tidx, ax, axx, drawplots, plot_interval, drawPheromonemap, drawUAVflight, drawAirspace, drawUAVconnectivity, connMat):  # BSedit
        if (drawplots and ((t % plot_interval) == 0)):
            ...
            if drawUAVflight:
                ...
                for i, bsID in enumerate(self.bs_iDs):
                    plt.plot([self.bs_position[i][0]/self.map_resolution], [self.bs_position[i][1]/self.map_resolution], marker='s')  # BSedit #SD*
